...
import yaml
import prior
from object_nav.envs.igridson.graphs import *
...
def gen_priors_graph(group_by_parent=True):
    ...
    # [1] ugly piece of logic to take the old-format hardcoded file and load it into a prior graph
    ...
    # [2] extracting probabilities from procthor-10k dataset
    print('Loading procthor-10k...')

    # try loading pickle object from storage if not exist load it and save to the storage
    try:
        with open('../priors/dataset.pkl', 'rb') as dataset_file:
            dataset = pickle.load(dataset_file)
    except FileNotFoundError:
        dataset = prior.load_dataset("procthor-10k")['train']
        with open('../priors/dataset.pkl', 'wb') as dataset_file:
    ...
    # [3] setting probabilities for each edge
    ...
    # [4] remove nodes with low edge counts
...
if __name__ == "__main__":
    ...
    print(f'Detailed priors graph has {len(graph.nodes)} nodes and {len(graph.get_edges())} edges')
    enrich_graph_with_metadata(graph)
    print(f'Detailed priors graph with metadata has {len(graph.nodes)} nodes and {len(graph.get_edges())} edges')
