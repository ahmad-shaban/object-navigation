...
class SMGFixedEnv(FixedEnv):
    def __init__(self, scene_sampler, scene_evolver, encode_obs_im=False, mission_mode='one_hot', scene=None,
                 set_goal_icon=False, env_evolve_freq=100):
    ...
    def get_num_objs(self):
        ...
        for node in self.scene.scene_graph.get_nodes_with_type(
                NodeType.FURNITURE) + self.scene.scene_graph.get_nodes_with_type(NodeType.OBJECT):
            ...
            for obj_node in furniture_node.get_children_nodes():
                if obj_node.type == NodeType.OBJECT:
                    ...
                    if len(edges) > 0:
                        ...
                        EDGE_TYPE_TO_FUNC[RECIPROCAL_EDGE_TYPES[edge.type].value](self, obj,
                                                                                  furniture)  # put the obj on the grid
                    else:
    ...
    def sample_to_grid(self, obj_node):
        if obj_node.type == NodeType.OBJECT:
            ...
            edge = \
                [e for e in obj_node.edges if
                 (e.node1.type == NodeType.FURNITURE or e.node2.type == NodeType.FURNITURE)][0]
