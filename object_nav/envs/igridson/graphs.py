...
def load_priors_graph(priors_type):
    ...
    if priors_type == 'detailed':
        graph_path = os.path.join(path, 'priors/detailed_prior_graph.pickle')
        prior_graph = load_graph(graph_path)
    elif priors_type == 'coarse':
        graph_path = os.path.join(path, 'priors/coarse_prior_graph.pickle')
        prior_graph = load_graph(graph_path)
    else:
...
class SGM(Graph):
    ...
    def save_pngs(self, path,
                  types=['priors', 'last_outputs', 'times_observed', 'freq_true', 'times_true', 'from_priors',
                         'from_observed']):
        for plot_type in types:
...
class SGMEdge(Edge):
    ...
    def copy(self, add_to_nodes=False):
        copy_edge = SGMEdge(self.node1, self.node2, self.type, self.prob, add_to_nodes=add_to_nodes,
                            from_priors=self.from_priors)
